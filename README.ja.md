...
<p align="center">
  <picture>
    ...
    <img src="docs/status.png" alt="進捗図: ステージ 0 完了。検証用 800 件でスキーマ・フリート・エスカレーション方向の妥当性はいずれも 100%、ルート一致率 91.4%、危険な下位振り分け 4%。ステージ 1〜3 はトレース収集と運用配備が存在しないため停止中。" width="680">
  </picture>
</p>

**現在地。** ステージ 0 は完了し、測定も済んでいる。構造的な保証は完全に成立した一方で、
主ルートの 4% が依然としてより*安価な*層へ倒れており、「上へ倒れる」不変条件はモデル単体
では成立していない（§6.1）。ステージ 1 と 2 にはまだデータが存在せず、何かを配備して観測を
始めるまで存在し得ない。トレース収集 → 推論提供 → 結果に基づく DPO がその循環であり、これが
回り始めるまではステージ 0 が上限である。進捗ボード:
...
├── datagen/       # 合成エンベロープ生成とルールオラクル（ステージ 0）
├── train/         # QLoRA のレシピ
├── eval/          # 評価ハーネスと結果
├── models/        # 学習済みアダプタ（Git LFS）
│   └── conductor-e2b-v0/   # ステージ 0 のアダプタ — 読み込み前に README を参照
├── common.py      # 学習と評価で共有するベースモデル読み込み処理
├── docs/          # アーキテクチャ図
├── harvest/       # 予定: トレース収集とエンベロープ変換（ステージ 1）
└── serve/         # 予定: MCP サーバー、OpenAI 互換シム、fail-up ガード
```

学習済みのステージ 0 アダプタは本リポジトリに同梱している:
**[models/conductor-e2b-v0](models/conductor-e2b-v0)**（Git LFS、92 MB）。読み込み前に
そちらの README を参照すること。ベースモデルからタワーを取り除いてからでないと、
PEFT はアダプタを適用できない。
